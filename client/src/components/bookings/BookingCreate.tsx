...
export function BookingCreate(props: Props) {
  const hook = useForm<BookingForm>({
    defaultValues: { book_in: props.book_in ? true : false },
  });
      ...
      {articles && (
        <Modal open={props.open} onClose={handleClose}>
          <Card sx={{ p: 2 }} className="modal-content">
            ...
            >
              {props.book_in ? "Einbuchung erstellen" : "Ausbuchung erstellen"}
            </Typography>
            <form onSubmit={hook.handleSubmit((values) => submit(values))}>
              <InputLabel style={{ color: "white" }} id="article-select">
                Artikel
              </InputLabel>
              <Select
