...
export function Article() {
  ...
  useEffect(() => {
    const getArticles = async () => {
      const response = await axios.get<PaginationResult<ArticleView[]>>(
        "/api/article/overview?page=" + page + "&q=" + searchParam
      );
      ...
      >
        ...
        <Box>
          <Button onClick={() => setOpenSearch(true)}>
            <ManageSearchIcon className="evo-green-text" />
          </Button>
      ...
      {articles && articles.length === 0 && (
        <Fragment>
          <Alert severity="info" sx={{ mt: 2 }}>
            {searchParam === "" && "Es wurden noch keine Artikel angelegt"}
        ...
        <Card sx={{ p: 2 }} className="modal-content">
          ...
          <form onSubmit={hook.handleSubmit((values) => submit(values))}>
            ...
            <Box display="flex" justifyContent="space-between">
              <Button
                variant="contained"
                className="evo-green-background"
