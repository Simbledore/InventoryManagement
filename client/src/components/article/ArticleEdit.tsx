...
import { ArticleForm, ArticleView } from "./article.models";
import {
  Alert,
  Box,
  Button,
...
export function ArticleEdit() {
  ...
  useEffect(() => {
    ...
    const getBookings = async () => {
      const response = await axios.get<PaginationResult<BookingView[]>>(
        "/api/booking/article/" + id + "?page=" + page
      );
    ...
    getArticle()
      ...
      .catch(() =>
        setLoadingError(
          "Beim Laden des Artikels ist ein Fehler aufgetrten, bitte kontaktieren Sie einen Administrator"
        )
      );

    getBookings().catch(() =>
      setLoadingError(
        "Beim Laden der zugehörigen Buchungen ist ein Fehler aufgetrten, bitte kontaktieren Sie einen Administrator"
      )
    );
  }, [open, page, id]);
    ...
    <Fragment>
      {article && (
        ...
        >
          ...
          >
            {article.name} - {article.amount} Stück
          </Typography>
          <Button onClick={() => setOpen(true)}>
            <EditIcon className="evo-green-text" />
          </Button>
        </Box>
      )}
      {bookings && (
      ...
      {bookings && bookings.length === 0 && (
        <Alert severity="info" sx={{ mt: 2 }}>
          Für diesen Artikel wurden noch keine Buchungen durhcgeführt
        </Alert>
      )}
      {loadingError && (
      ...
      <Modal open={open} onClose={handleClose}>
        <Card sx={{ p: 2 }} className="modal-content">
          <Typography
            variant="h2"
            sx={{ fontSize: { xs: "25px", sm: "35px" }, mb: 1 }}
          >
            Artikel bearbeiten
          </Typography>
